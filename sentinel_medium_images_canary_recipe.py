#!/usr/bin/env python3
"""MEDIUM Images Canary Recipe Gate (Phase 8.11).
...
ALLOWED_REMOTE_PATH = "/wordpress/wp-content/uploads/2023/08/Acid-Love-Cover-Art-1-2.webp"
ALLOWED_IMAGE_URL = "https://electri-c-ity-studios-24-7.com/wp-content/uploads/2023/08/Acid-Love-Cover-Art-1-2.webp"
BACKUP_FILE = PROJECT_DIR / "backups/medium-images-apply/20260613-175842/copied/wordpress/wp-content/uploads/2023/08/Acid-Love-Cover-Art-1-2.webp"
EXPECTED_BACKUP_SHA256 = "f6ab58160164e10969a13385c26de5185239cca920e4722f5c5225cb7586936f"
TARGET_URL = "https://electri-c-ity-studios-24-7.com/"

REPORT_JSON = PROJECT_DIR / "reports/latest/medium-images-canary-recipe.json"
REPORT_MD = PROJECT_DIR / "reports/latest/medium-images-canary-recipe.md"
TOOLING_JSON = PROJECT_DIR / "reports/latest/medium-images-canary-tooling-check.json"
PRE_HEALTHCHECK_JSON = PROJECT_DIR / "reports/latest/medium-images-canary-pre-upload-healthcheck.json"
POST_HEALTHCHECK_JSON = PROJECT_DIR / "reports/latest/medium-images-canary-post-upload-healthcheck.json"
VALIDATION_JSON = PROJECT_DIR / "reports/latest/medium-images-canary-validation.json"
ROLLBACK_JSON = PROJECT_DIR / "reports/latest/medium-images-canary-rollback.json"
FINAL_REPORT_MD = PROJECT_DIR / "reports/latest/medium-images-canary-final-report.md"
SNAPSHOT_DIR = PROJECT_DIR / "snapshots"
AUDIT_JSONL = PROJECT_DIR / "audit/medium-images-canary-recipe.jsonl"

STATE_JSON = PROJECT_DIR / "state/adaptive-learning/medium_images_canary_recipe_state.json"
LATEST_JSON = PROJECT_DIR / "state/adaptive-learning/latest_medium_images_canary_recipe.json"
...
CANARY_ROOT = PROJECT_DIR / "backups/medium-images-canary-recipe"

PLAYBOOK_RECIPE = PROJECT_DIR / "playbooks/medium-images-canary-recipe.playbook.json"
PLAYBOOK_ROLLBACK = PROJECT_DIR / "playbooks/medium-images-canary-rollback.playbook.json"
PLAYBOOK_VALIDATION = PROJECT_DIR / "playbooks/medium-images-canary-validation.playbook.json"
...
SCHEMA_VERSION = "medium-images-canary-recipe-8.11"

STATUS_TOOLING_MISSING = "MEDIUM_IMAGES_CANARY_TOOLING_MISSING"
STATUS_READY = "MEDIUM_IMAGES_CANARY_READY"
STATUS_LOCAL_OPTIMIZED = "MEDIUM_IMAGES_CANARY_LOCAL_OPTIMIZED"
STATUS_UPLOAD_EXECUTED = "MEDIUM_IMAGES_CANARY_UPLOAD_EXECUTED"
STATUS_UPLOAD_SKIPPED_NOT_BENEFICIAL = "MEDIUM_IMAGES_CANARY_UPLOAD_SKIPPED_NOT_BENEFICIAL"
STATUS_UPLOAD_BLOCKED = "MEDIUM_IMAGES_CANARY_UPLOAD_BLOCKED_BY_SAFETY"
STATUS_VALIDATION_OK = "MEDIUM_IMAGES_CANARY_VALIDATION_OK"
STATUS_VALIDATION_DEGRADED = "MEDIUM_IMAGES_CANARY_VALIDATION_DEGRADED"
STATUS_ROLLBACK_COMPLETED = "ROLLBACK_COMPLETED"
STATUS_ROLLBACK_NOT_REQUIRED = "ROLLBACK_NOT_REQUIRED"
STATUS_FAILED = "MEDIUM_IMAGES_CANARY_FAILED"
...
class HealthParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: List[Tuple[str, Optional[str]]]) -> None:
        ...
        if tag == "title":
            ...
            if "Acid-Love-Cover-Art-1-2.webp" in combined:
...
def backup_ok() -> Dict[str, Any]:
    exists = BACKUP_FILE.exists()
    sha = sha256_file(BACKUP_FILE) if exists else None
    size = BACKUP_FILE.stat().st_size if exists else None
    return {
        "backup_path": str(BACKUP_FILE),
        "exists": exists,
        "sha256": sha,
        "expected_sha256": EXPECTED_BACKUP_SHA256,
        "sha256_matches": sha == EXPECTED_BACKUP_SHA256,
        "size_bytes": size,
    }
...
def fetch_home_healthcheck() -> Dict[str, Any]:
    ...
    request = urllib.request.Request(TARGET_URL, headers={"User-Agent": "SentinelImagesCanaryHealthcheck/8.11"})
    try:
        with urllib.request.urlopen(request, timeout=20) as response:
            ...
            return {
                ...
                "target_image_present": parser.target_image_present or "Acid-Love-Cover-Art-1-2.webp" in html,
                "headers_subset": {
...
def fetch_image_healthcheck() -> Dict[str, Any]:
    request = urllib.request.Request(ALLOWED_IMAGE_URL, headers={"User-Agent": "SentinelImagesCanaryImageProbe/8.11"})
    try:
        with urllib.request.urlopen(request, timeout=20) as response:
...
def recommended_owner_action(status: str, state: Dict[str, Any]) -> str:
    if status == STATUS_TOOLING_MISSING:
...
def write_common(report: Dict[str, Any], state: Dict[str, Any]) -> None:
    snapshot = SNAPSHOT_DIR / f"medium-images-canary-recipe-{report['timestamp']}.json"
...
def write_playbooks(report: Dict[str, Any]) -> None:
    common = {
        ...
        "backup_sha256_required": EXPECTED_BACKUP_SHA256,
        "minimum_savings_percent": 3.0,
        "global_live_autonomy": False,
    }
    write_json_atomic(PLAYBOOK_RECIPE, {
        **common,
        "name": "medium-images-canary-recipe",
    ...
    write_json_atomic(PLAYBOOK_ROLLBACK, {
        **common,
        "name": "medium-images-canary-rollback",
        "rollback_remote_path": ALLOWED_REMOTE_PATH,
        "rollback_from_backup": str(BACKUP_FILE),
    })
    write_json_atomic(PLAYBOOK_VALIDATION, {
        **common,
        "name": "medium-images-canary-validation",
...
def update_learning(report: Dict[str, Any]) -> None:
    ...
    update_json_file(PATTERNS_JSON, {
        "medium_images_canary_recipe_pattern": {
    ...
    update_json_file(ADAPTIVE_LATEST_JSON, {
        "latest_medium_images_canary_status": report.get("status"),
        "latest_medium_images_canary_timestamp": timestamp,
        "latest_medium_images_canary_breach": report.get("breach"),
    })
    ...
    section = (
        "\n\n## Phase 8.11 MEDIUM Images Canary Recipe\n"
        f"- status: `{report.get('status')}`\n"
        f"- upload_executed: `{report.get('upload_executed')}`\n"
...
def tooling_check_action() -> Dict[str, Any]:
    state = load_state()
    tooling = tooling_status()
    status = STATUS_READY if tooling["minimal_tooling_ok"] else STATUS_TOOLING_MISSING
    state.setdefault("stage_statuses", {})["tooling_check"] = status
    state["tooling"] = tooling
    write_json_atomic(TOOLING_JSON, tooling)
    report = base_report("tooling-check", status, state, {"tooling": tooling})
    write_common(report, state)
    return report


def prepare_canary_action() -> Dict[str, Any]:
    state = load_state()
    ...
    if backup["exists"] and not backup["sha256_matches"]:
        status = STATUS_UPLOAD_BLOCKED
    ...
    if status == STATUS_READY:
        shutil.copyfile(BACKUP_FILE, original)
        state["original_path"] = str(original)
    state.setdefault("stage_statuses", {})["prepare_canary"] = status
    state["work_dir"] = str(work_dir)
    state["backup_check"] = backup
    state["breach"] = status == STATUS_UPLOAD_BLOCKED
    state["breach_reasons"] = reasons
    report = base_report("prepare-canary", status, state, {"backup_check": backup, "work_dir": str(work_dir), "block_reasons": reasons})
    write_common(report, state)
    return report
...
def optimize_local_canary_action() -> Dict[str, Any]:
    ...
    if not tooling.get("minimal_tooling_ok"):
        status = STATUS_TOOLING_MISSING
        state.setdefault("stage_statuses", {})["optimize_local_canary"] = status
    ...
    if not original.exists():
        ...
        report = base_report("optimize-local-canary", status, state, {"reason": "original_canary_copy_missing"})
        write_common(report, state)
        return report
    work_dir = current_work_dir(state)
    output = work_dir / "optimized.webp"
    before = webpinfo(original)
    direct = run_tool(["cwebp", "-quiet", "-q", "82", "-m", "6", "-mt", str(original), "-o", str(output)])
    recipe = "direct_cwebp_webp_to_webp"
    if not direct.get("ok") and tooling.get("tools_present", {}).get("dwebp"):
    ...
    state.setdefault("stage_statuses", {})["optimize_local_canary"] = status
    report = base_report("optimize-local-canary", status, state, {"optimization": optimization})
    write_common(report, state)
    return report
...
def pre_upload_healthcheck_action() -> Dict[str, Any]:
    ...
    check["healthcheck_status"] = "PRE_UPLOAD_HEALTHCHECK_OK" if healthcheck_ok(check) and check["image_probe"].get("http_status") == 200 and not check["low_risk_breach"] else STATUS_UPLOAD_BLOCKED
...
def upload_canary_action() -> Dict[str, Any]:
    ...
    if not owner_decision_ok():
    ...
    elif not (state.get("backup_check") or {}).get("sha256_matches"):
        ...
        elif before.get("sha256") != EXPECTED_BACKUP_SHA256:
...
def rollback_if_degraded_action() -> Dict[str, Any]:
    ...
    if not state.get("upload_executed"):
        ...
        if not sftp_env_ready():
            ...
            upload = upload_remote_exact(BACKUP_FILE)
            remote_after = download_remote_exact(current_work_dir(state) / "remote-after-rollback.webp") if upload.get("ok") else {}
            status = STATUS_ROLLBACK_COMPLETED if remote_after.get("sha256") == EXPECTED_BACKUP_SHA256 else STATUS_UPLOAD_BLOCKED
...
def print_summary(report: Dict[str, Any]) -> None:
    print(f"status={report.get('status') or report.get('final_status')}")
...
def run_self_test() -> int:
    ...
    missing_backup = {"exists": False, "sha256_matches": False}
    if missing_backup["exists"]:
        raise AssertionError("missing backup not represented")
    wrong_hash = {"exists": True, "sha256_matches": False}
    if wrong_hash["sha256_matches"]:
        raise AssertionError("wrong hash not represented")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MEDIUM images canary recipe gate.")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--self-test", action="store_true")
    group.add_argument("--tooling-check", action="store_true")
    group.add_argument("--prepare-canary", action="store_true")
    group.add_argument("--optimize-local-canary", action="store_true")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.status:
        ...
        return 0
    try:
        if args.tooling_check:
            report = tooling_check_action()
        elif args.prepare_canary:
            report = prepare_canary_action()
        elif args.optimize_local_canary:
