#!/usr/bin/env python3
"""MEDIUM Images Next Canary Candidate Selector (Phase 8.12).
...
candidates, blocks the previous under-threshold canary, and writes owner review
packs for the next separate recipe phase.
"""
...
REPORT_JSON = PROJECT_DIR / "reports/latest/medium-images-next-canary-selector.json"
REPORT_MD = PROJECT_DIR / "reports/latest/medium-images-next-canary-selector.md"
OWNER_PACK_MD = PROJECT_DIR / "reports/latest/medium-images-next-canary-owner-pack.md"
BACKUP_PLAN_MD = PROJECT_DIR / "reports/latest/medium-images-next-canary-backup-plan.md"
RECIPE_PLAN_MD = PROJECT_DIR / "reports/latest/medium-images-next-canary-recipe-plan.md"
SNAPSHOT_DIR = PROJECT_DIR / "snapshots"
AUDIT_JSONL = PROJECT_DIR / "audit/medium-images-next-canary-selector.jsonl"

STATE_JSON = PROJECT_DIR / "state/adaptive-learning/medium_images_next_canary_selector.json"
LATEST_JSON = PROJECT_DIR / "state/adaptive-learning/latest_medium_images_next_canary.json"

PLAYBOOK_SELECTOR = PROJECT_DIR / "playbooks/medium-images-next-canary-selector.playbook.json"
PLAYBOOK_RECIPE = PROJECT_DIR / "playbooks/medium-images-next-canary-recipe.playbook.json"
...
INPUTS = {
    ...
    "canary_recipe": PROJECT_DIR / "reports/latest/medium-images-canary-recipe.json",
    "canary_validation": PROJECT_DIR / "reports/latest/medium-images-canary-validation.json",
    "latest_canary_recipe": PROJECT_DIR / "state/adaptive-learning/latest_medium_images_canary_recipe.json",
...
SCHEMA_VERSION = "medium-images-next-canary-selector-8.12"
STATUS_OK = "MEDIUM_IMAGES_NEXT_CANARY_SELECTOR_OK"
STATUS_WARNINGS = "MEDIUM_IMAGES_NEXT_CANARY_SELECTOR_WARNINGS"
STATUS_NO_SAFE = "MEDIUM_IMAGES_NEXT_CANARY_NO_SAFE_CANDIDATE"
STATUS_BLOCKED = "MEDIUM_IMAGES_NEXT_CANARY_BLOCKED_BY_SAFETY"
STATUS_FAILED = "MEDIUM_IMAGES_NEXT_CANARY_FAILED"
...
def fetch_live_images() -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    request = urllib.request.Request(TARGET_URL, headers={"User-Agent": "SentinelNextCanarySelector/8.12"})
    try:
...
def report_candidates(inputs: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for source_name in ("dryrun_report", "dryrun_state"):
...
def size_probe(url: str) -> Dict[str, Any]:
    headers = {"User-Agent": "SentinelNextCanarySizeProbe/8.12"}
...
def merge_candidates(report_rows: List[Dict[str, Any]], live_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for url, item in merged.items():
        ...
        out.append({
            ...
            "is_previous_failed_candidate": remote_path == PREVIOUS_FAILED_REMOTE or url == PREVIOUS_FAILED_URL,
...
def classify_and_rank(candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    larger_safe_exists = any(
        (c.get("size_bytes") or 0) >= 150_000
        and c.get("source_type") == "internal"
        and c.get("remote_path")
        and not c.get("is_previous_failed_candidate")
    ...
    for c in candidates:
        ...
        if larger_safe_exists and size < 150_000:
            blocked.append("larger_safe_candidate_available")
...
def render_report_md(report: Dict[str, Any]) -> str:
    ...
    lines = [
        "# MEDIUM Images Next Canary Selector",
        "",
...
def render_owner_pack_md(report: Dict[str, Any]) -> str:
    selected = report.get("selected_candidate") or {}
    return "\n".join([
        "# MEDIUM Images Next Canary Owner Pack",
        "",
...
def render_backup_plan_md(report: Dict[str, Any]) -> str:
    selected = report.get("selected_candidate") or {}
    return "\n".join([
        "# MEDIUM Images Next Canary Backup Plan",
        "",
        f"- remote_path: `{selected.get('remote_path', '-')}`",
...
def render_recipe_plan_md(report: Dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# MEDIUM Images Next Canary Recipe Plan",
        "",
...
def playbook_selector(report: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "name": "medium-images-next-canary-selector",
        "phase": "8.12",
        "purpose": "Read-only selection of a better next images canary candidate.",
...
def playbook_recipe(report: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "name": "medium-images-next-canary-recipe",
        "phase": "future",
...
def write_outputs(report: Dict[str, Any], write_pack: bool = False) -> None:
    snapshot = SNAPSHOT_DIR / f"medium-images-next-canary-selector-{report['timestamp']}.json"
...
def update_learning(report: Dict[str, Any]) -> None:
    ...
    update_json_file(KNOWLEDGE_BASE_JSON, {
        "medium_images_next_canary_selector": {
    ...
    update_json_file(PATTERNS_JSON, {
        "medium_images_next_canary_pattern": {
    ...
    update_json_file(ACTION_RULES_JSON, {
        "medium_images_next_canary_rules": {
    ...
    update_json_file(ADAPTIVE_LATEST_JSON, {
        "latest_medium_images_next_canary_status": report.get("status"),
        "latest_medium_images_next_canary_timestamp": timestamp,
        "latest_medium_images_next_canary_remote_path": selected.get("remote_path"),
    })
    append_jsonl(OBSERVATIONS_JSONL, [{
        "timestamp_utc": timestamp,
        "source": SCHEMA_VERSION,
        "observation": "Next image canary candidate selected read-only after previous under-threshold canary was blocked from reuse.",
        "status": report.get("status"),
        "selected_remote_path": selected.get("remote_path"),
        "breach": report.get("breach"),
    }])
    section = (
        "\n\n## Phase 8.12 MEDIUM Images Next Canary Selector\n"
        f"- status: `{report.get('status')}`\n"
        f"- selected_remote_path: `{selected.get('remote_path', '-')}`\n"
        "- Previous under-threshold Canary is blocked from reuse.\n"
...
def owner_pack_action() -> Dict[str, Any]:
    report = build_report("owner-next-canary-pack", write_pack=True)
...
def run_self_test() -> int:
    ...
    for row in rows:
        row["is_previous_failed_candidate"] = row["remote_path"] == PREVIOUS_FAILED_REMOTE
...
def build_parser() -> argparse.ArgumentParser:
    ...
    group.add_argument("--owner-next-canary-pack", action="store_true")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        elif args.owner_next_canary_pack:
